...
const executeMigration = async function(queryInterface, filename, useTransaction, pos, rollback)
{
    let mig = require(filename);
    
    if (!mig) {
        console.log("executeMigration error:", "Can't require file "+filename)
        return false;
    }
    ...
    if (rollback) {
        if (typeof mig.down !== 'function') {
            console.log("executeMigration error:", "No rollback command")
            return false;
        }

        try {
            await mig.down(queryInterface, Sequelize)
            return true;
        } catch (err){
            console.log("executeMigration error:", err)
            return false;
        }
    } else {
        try {
